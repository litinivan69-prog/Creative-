...
const EMPTY_BRIEF: BriefValues = {
  brandName: "", website: "", businessDescription: "", priorityOffer: "", audience: "Уточним позже в чате",
  ...
  starterKitPlatformIds: [], brandColors: "", fonts: "", visualStyle: "Уточним позже по материалам бренда", likedVisualReferences: "",
...
const flowSteps = [1, 2, 3, 4, 11] as const;
const stepLabels = ["Сайт", "Название", "Бизнес", "Продукт", "Готово"];
...
export function SelfServiceBrief({ selection, resetDraft = false }: { selection: SelfServiceSelection; resetDraft?: boolean }) {
  ...
  useEffect(() => {
    try {
      if (resetDraft) {
        ...
        if ((flowSteps as readonly number[]).includes(savedStep)) setStep(savedStep);
      }
  ...
  const flowIndex = Math.max(0, flowSteps.indexOf(step as (typeof flowSteps)[number]));
  const next = () => { if (canContinue) setStep(flowSteps[Math.min(flowSteps.length - 1, flowIndex + 1)]); };
  const back = () => setStep(flowSteps[Math.max(0, flowIndex - 1)]);
  ...
  const progress = Math.round(((flowIndex + 1) / flowSteps.length) * 100);
  return <main className="relative min-h-screen overflow-hidden bg-[#09090d] px-4 py-5 text-white sm:px-7 lg:px-10"><div className="pointer-events-none absolute inset-x-0 top-0 h-[620px] bg-[radial-gradient(circle_at_18%_0%,rgba(124,92,255,.18),transparent_38%),radial-gradient(circle_at_85%_12%,rgba(77,208,168,.05),transparent_28%)]" /><div className="relative mx-auto max-w-4xl">
    <header className="flex items-center justify-between border-b border-white/[.08] py-3"><a href="/start"><RibesBrand dark /></a><span className="text-xs font-semibold text-slate-500">{flowIndex + 1} из {flowSteps.length}</span></header>
    <section className="py-8 sm:py-12"><div className="h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-violet-500 transition-all duration-300" style={{ width: `${progress}%` }} /></div><div className="mt-3 flex items-center justify-between text-[10px] font-semibold uppercase tracking-[.1em]"><span className="text-violet-300">{stepLabels[flowIndex]}</span><span className="text-slate-600">{progress}% заполнено</span></div>
      <div className="mt-8 min-h-[500px] rounded-[30px] border border-white/[.09] bg-[#101015]/95 p-5 shadow-[0_28px_100px_rgba(0,0,0,.36)] sm:p-8">
        ...
        {step === 11 ? <div><h1 className="font-heading text-3xl font-semibold tracking-[-.04em] sm:text-5xl">Этого достаточно для старта</h1><p className="mt-4 text-sm text-slate-400">Остальное можно рассказать позже в чате «Мозг бренда». Сначала попадите в кабинет и попробуйте продукт.</p><div className="mt-7 grid gap-3 sm:grid-cols-2">{[["Бренд",values.brandName],["Чем занимается",values.businessDescription],["Продвигаем",values.priorityOffer],["Сайт",values.website || "Без сайта"]].map(([label,value]) => <div key={label} className="rounded-2xl border border-white/[.08] bg-white/[.025] p-4"><p className="text-[9px] font-bold uppercase tracking-[.12em] text-slate-600">{label}</p><p className="mt-2 text-sm leading-5 text-white/68">{value}</p></div>)}</div><form action={stageSelfServiceOnboarding} className="mt-5"><input type="hidden" name="selection" value={JSON.stringify(selection)} /><input type="hidden" name="brief" value={JSON.stringify({ ...values, audience: values.audience || "Уточним позже в чате", visualStyle: values.visualStyle || "Уточним позже по материалам бренда" })} /><label className="mb-5 flex items-start gap-3 rounded-2xl border border-white/[.08] bg-white/[.025] p-4"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} className="mt-0.5 h-4 w-4 accent-violet-500" /><span className="text-xs leading-5 text-slate-400">Данные верны. Я принимаю <Link href="/legal/terms" target="_blank" className="text-violet-300 underline">условия использования</Link> и даю <Link href="/legal/consent" target="_blank" className="text-violet-300 underline">согласие на обработку данных</Link>.</span></label><CreateButton confirmed={confirmed} /></form></div> : null}
        <div className="mt-9 flex items-center justify-between border-t border-white/[.08] pt-6"><button type="button" onClick={back} disabled={flowIndex === 0} className="rounded-full px-5 py-3 text-sm font-semibold text-slate-400 hover:text-white disabled:invisible">Назад</button>{flowIndex < flowSteps.length - 1 ? <button type="button" onClick={next} disabled={!canContinue} className="rounded-full bg-violet-500 px-7 py-3.5 text-sm font-semibold text-white transition hover:bg-violet-400 disabled:cursor-not-allowed disabled:opacity-35">Дальше</button> : null}</div>
      </div>
    </section>
  </div></main>;
}
