...
type Configuration = {
  ...
  dzenArticleTopic: string;
  vcruArticleTopic: string;
};

type NumericConfigurationKey = Exclude<keyof Configuration, "carouselPlatform" | "carouselSlides" | "dzenArticleTopic" | "vcruArticleTopic">;
...
const items: Array<{ key: NumericConfigurationKey; title: string; detail: string; credits: number; icon: string }> = [
  ...
  { key: "carousels", title: "Карусели", detail: "От одного до четырёх отдельных слайдов", credits: CREDIT_PRODUCTS.carousel.credits, icon: "▦" },
];
...
function LaunchButton({ hasExistingPlan, unlimited, total, disabled }: { hasExistingPlan: boolean; unlimited: boolean; total: number; disabled: boolean }) {
  const { pending } = useFormStatus();
  const action = hasExistingPlan ? addSelfServiceContentToCurrentMonth : launchSelfServiceContentOrder;
  const idleLabel = unlimited
    ? hasExistingPlan ? "Добавить без списания" : "Запустить без списания"
    : `${hasExistingPlan ? "Добавить" : "Запустить"} за ${displayCredits(total)} кредитов`;

  return <button formAction={action} disabled={disabled || pending} className="mt-6 w-full rounded-xl bg-white px-5 py-3.5 text-xs font-semibold text-slate-950 transition hover:bg-violet-50 disabled:cursor-wait disabled:opacity-35">{pending ? hasExistingPlan ? "Добавляем в календарь…" : "Запускаем подготовку…" : idleLabel}</button>;
}

export function ContentMixBuilder({ balance, unlimited = false, initial, notice, error, hasExistingPlan = false }: { balance: number; unlimited?: boolean; initial: Configuration; notice?: string; error?: string; hasExistingPlan?: boolean }) {
  ...
  return (
    <form action={saveSelfServiceContentOrder} className="grid gap-5 lg:grid-cols-[1fr_330px]">
      <section className="space-y-3">
        ...
        {error === "addition_failed" ? <p className="rounded-2xl border border-rose-300/15 bg-rose-300/[0.06] px-4 py-3 text-xs text-rose-100/80">Не удалось добавить материалы с первого раза. Заказ сохранён — попробуйте ещё раз или откройте календарь.</p> : null}
        ...
        {items.map((item) => {
          const topicKey: "dzenArticleTopic" | "vcruArticleTopic" | null = item.key === "dzenArticles" ? "dzenArticleTopic" : item.key === "vcruArticles" ? "vcruArticleTopic" : null;
          return <article id={item.key} key={item.key} className="scroll-mt-24 rounded-[20px] border border-white/[0.07] bg-white/[0.025] p-4 sm:p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex min-w-0 items-center gap-3.5">{itemPlatforms[item.key] ? <PlatformBrandIcon platform={itemPlatforms[item.key]!} size="sm" /> : <span className="grid h-9 w-9 shrink-0 place-items-center rounded-[11px] bg-violet-500/12 text-[13px] font-bold text-violet-200">▦</span>}<div><p className="text-sm font-semibold text-white/82">{item.title}</p><p className="mt-1 text-[10px] text-white/28">{item.detail} · {displayCredits(item.credits)} кредитов</p></div></div><Counter value={configuration[item.key]} onChange={(value) => setConfiguration((current) => ({ ...current, [item.key]: value }))} /><input type="hidden" name={item.key} value={configuration[item.key]} /></div>{topicKey && configuration[item.key] > 0 ? <label className="mt-4 block border-t border-white/[0.06] pt-4"><span className="text-[10px] font-semibold text-white/42">Точная тема первой статьи · необязательно</span><input name={topicKey} value={configuration[topicKey]} onChange={(event) => setConfiguration((current) => ({ ...current, [topicKey]: event.target.value }))} maxLength={500} placeholder="Например: как выбрать кухонную мойку и не ошибиться с монтажом" className="mt-2 w-full rounded-xl border border-white/[0.08] bg-black/20 px-4 py-3 text-xs text-white/75 outline-none placeholder:text-white/20 focus:border-violet-400/35" /></label> : null}{item.key === "carousels" && configuration.carousels > 0 ? <div className="mt-4 space-y-3 border-t border-white/[0.06] pt-4"><div className="flex flex-wrap items-center gap-2"><span className="mr-2 text-[10px] text-white/32">Слайдов:</span>{([1, 2, 3, 4] as const).map((count) => <button key={count} type="button" onClick={() => setConfiguration((current) => ({ ...current, carouselSlides: count }))} className={`grid h-8 w-8 place-items-center rounded-full border text-[10px] font-semibold transition ${configuration.carouselSlides === count ? "border-violet-400/40 bg-violet-500/15 text-violet-200" : "border-white/[0.08] text-white/35 hover:text-white"}`}>{count}</button>)}</div><div className="flex flex-wrap items-center gap-2"><span className="mr-2 text-[10px] text-white/32">Куда публикуем:</span>{(["vk", "telegram", "both"] as const).map((platform) => <button key={platform} type="button" onClick={() => setConfiguration((current) => ({ ...current, carouselPlatform: platform }))} className={`flex items-center gap-2 rounded-full border px-3 py-2 text-[10px] font-semibold transition ${configuration.carouselPlatform === platform ? "border-violet-400/40 bg-violet-500/15 text-violet-200" : "border-white/[0.08] text-white/35 hover:text-white"}`}>{platform === "both" ? <span className="flex -space-x-1.5"><PlatformBrandIcon platform="VK" size="xs" /><PlatformBrandIcon platform="Telegram" size="xs" /></span> : <PlatformBrandIcon platform={platform === "vk" ? "VK" : "Telegram"} size="xs" />}{platform === "vk" ? "VK" : platform === "telegram" ? "Telegram" : "VK + Telegram"}</button>)}</div></div> : null}</article>;
        })}
        <input type="hidden" name="carouselPlatform" value={configuration.carouselPlatform} />
        <input type="hidden" name="carouselSlides" value={configuration.carouselSlides} />
      </section>

      <aside className="h-fit rounded-[24px] border border-violet-400/15 bg-[linear-gradient(145deg,rgba(111,75,255,.14),rgba(255,255,255,.025))] p-5 lg:sticky lg:top-24 sm:p-6"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-violet-300">{hasExistingPlan ? "Добавление" : "Ваш набор"}</p><div className="mt-5 flex items-end justify-between"><div><p className="text-4xl font-semibold tracking-[-0.05em] text-white">{displayCredits(total)}</p><p className="mt-1 text-[10px] text-white/28">кредитов потребуется</p></div><div className="text-right"><p className="text-lg font-semibold text-white/75">{unlimited ? "Безлимит" : displayCredits(balance)}</p><p className="mt-1 text-[9px] text-white/25">{unlimited ? "администратор" : "сейчас на балансе"}</p></div></div><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/[0.07]"><div className={`h-full rounded-full ${enough ? "bg-violet-500" : "bg-amber-300/70"}`} style={{ width: `${unlimited ? 100 : Math.min(100, balance ? (total / balance) * 100 : 100)}%` }} /></div><p className={`mt-3 text-[10px] leading-4 ${enough ? "text-white/32" : "text-amber-100/70"}`}>{total === 0 ? "Добавьте нужные материалы — стоимость считается сразу." : unlimited ? "Админский тест: запуск сохранится в истории, кредиты не спишутся." : enough ? `После запуска останется ${displayCredits(balance - total)} кредитов.` : `Не хватает ${displayCredits(total - balance)} кредитов. Набор всё равно можно сохранить.`}</p><LaunchButton hasExistingPlan={hasExistingPlan} unlimited={unlimited} total={total} disabled={!enough || total === 0} />{!hasExistingPlan ? <button formAction={saveSelfServiceContentOrder} className="mt-2 w-full rounded-xl border border-white/[0.08] px-5 py-3 text-xs font-semibold text-white/55 transition hover:bg-white/[0.04] hover:text-white">Сохранить без запуска</button> : null}<p className="mt-3 text-center text-[9px] text-white/22">{hasExistingPlan ? "Добавятся только выбранные позиции. Готовые материалы не пересобираются" : unlimited ? "Расход OpenAI учитывается отдельно" : "Кредиты списываются только при запуске"}</p>{!unlimited ? <a href="/app/credits" className="mt-4 block text-center text-[10px] font-semibold text-violet-300">Тарифы и пополнение →</a> : null}</aside>
    </form>
  );
}
