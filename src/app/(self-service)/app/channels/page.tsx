...
export default async function SelfServiceChannelsPage({ searchParams }: { searchParams: Promise<{ notice?: string; error?: string; from?: string }> }) {
  ...
  return (
    <SelfServiceAppShell
      ...
      description={isOnboarding ? "Подключение не обязательно для знакомства с платформой. Без него мы всё равно подготовим материалы — автоматическая публикация включится позже." : "VK и Telegram можно публиковать из кабинета. Одноклассники, Дзен и VC.ru на старте получают готовые материалы для ручного размещения."}
    >
      <div className="max-w-[960px]">
        ...
        <section className="mt-7">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-violet-300">Автопостинг</p><h2 className="mt-2 text-xl font-semibold tracking-[-0.025em] text-white">VK и Telegram</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-white/30">Подключите одну или обе площадки. Система отправит только тот материал, который вы сами подтвердили.</p></div><span className="rounded-full border border-white/[0.06] bg-white/[0.03] px-3 py-1.5 text-[9px] text-white/30">можно настроить позже</span></div>
        ...
        <section className="mt-8 border-t border-white/[0.06] pt-8">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-violet-300">Статьи</p><h2 className="mt-2 text-xl font-semibold tracking-[-0.025em] text-white">Дзен и VC.ru — без API</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-white/30">Платформа готовит структуру, полный текст, обложку и DOCX. Вы размещаете статью в своём аккаунте и отмечаете публикацию — она попадает в отчёт.</p></div>
          <div className="mt-5 grid gap-4 lg:grid-cols-2">
            {(["Дзен", "VC.ru"] as PlatformBrand[]).map((platform) => <article key={platform} className="rounded-[22px] border border-white/[0.07] bg-white/[0.03] p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-3.5"><PlatformBrandIcon platform={platform} size="sm" /><div><p className="text-base font-semibold text-white">{platform}</p><p className="mt-1 text-[10px] text-white/28">готовая статья для ручного размещения</p></div></div><span className="rounded-full bg-white/[0.05] px-3 py-1.5 text-[9px] font-semibold text-white/35">без токена</span></div><div className="mt-5 grid grid-cols-3 gap-2 text-center">{[["01", "Проверить"], ["02", "Скачать"], ["03", "Разместить"]].map(([number, label]) => <div key={number} className="rounded-xl border border-white/[0.05] bg-black/10 px-2 py-3"><p className="text-[9px] font-semibold text-violet-300">{number}</p><p className="mt-1 text-[9px] text-white/38">{label}</p></div>)}</div></article>)}
          </div>
        ...
        {isOnboarding ? <section className="sticky bottom-4 mt-5 rounded-[22px] border border-white/[0.08] bg-[#111014]/95 p-4 shadow-[0_24px_80px_rgba(0,0,0,.45)] backdrop-blur-xl sm:flex sm:items-center sm:justify-between sm:gap-5"><div><p className="text-xs font-semibold text-white/72">{connectedCount ? `Автопостинг подключён: ${connectedCount} ${connectedCount === 1 ? "площадка" : "площадки"}` : "Материалы можно подготовить без подключения"}</p><p className="mt-1 text-[10px] leading-4 text-white/28">VK и Telegram всегда доступны здесь. Дзен и VC.ru не требуют токенов.</p></div><form action={completeSelfServiceChannelOnboarding} className="mt-3 sm:mt-0"><button className="w-full rounded-xl bg-violet-500 px-5 py-3 text-xs font-semibold text-white transition hover:bg-violet-400 sm:w-auto">{connectedCount ? "Продолжить — собрать месяц" : "Продолжить без подключения"}</button></form></section> : null}
      </div>
    </SelfServiceAppShell>
  );
}
