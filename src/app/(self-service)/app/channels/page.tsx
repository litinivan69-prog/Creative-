...
import { isOkOauthConfigured } from "@/lib/ok-oauth";
...
export default async function SelfServiceChannelsPage({ searchParams }: { searchParams: Promise<{ notice?: string; error?: string; from?: string }> }) {
  ...
  const socialDefinitions = [
    ...
    { platform: "vk" as const, label: "VK", description: "Публикация полного текста и изображений в сообщество.", referencePlaceholder: "https://vk.com/сообщество", tokenPlaceholder: "Вставьте ключ сообщества", platformToken: platformVkToken, platformHint: platformVkLabel, oauthAvailable: true },
  ...
  const channels = socialDefinitions.map((definition) => {
    ...
    const connected = saved?.status === "active"
      && (Boolean(saved.credentialEncrypted) || definition.platformToken)
      && (definition.platform !== "vk" || saved?.credentialHint?.startsWith("Ключ сообщества"));
    return {
      ...definition,
      id: saved?.id ?? null,
  ...
  return (
    <SelfServiceAppShell
      ...
      description={isOnboarding ? "Подключение не обязательно для знакомства с платформой. Без него мы всё равно подготовим материалы — автоматическая публикация включится позже." : "VK, Telegram, VC.ru и Одноклассники публикуются из кабинета. Подключение Дзена готовится к закрытому тестированию."}
    >
      <div className="max-w-[960px]">
        ...
        <section className="mt-8 border-t border-white/[0.06] pt-8">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-[18px] border border-white/[0.06] bg-black/15 px-4 py-3.5"><p className="text-[10px] leading-4 text-white/28">Статьи остаются доступны для скачивания, пока автоматическое подключение Дзена проходит проверку.</p><Link href="/app/articles" className="text-[10px] font-semibold text-violet-300 transition hover:text-violet-200">Открыть статьи →</Link></div>
        </section>
        {isOnboarding ? <section className="sticky bottom-4 mt-5 rounded-[22px] border border-white/[0.08] bg-[#111014]/95 p-4 shadow-[0_24px_80px_rgba(0,0,0,.45)] backdrop-blur-xl sm:flex sm:items-center sm:justify-between sm:gap-5"><div><p className="text-xs font-semibold text-white/72">{connectedCount ? `Автопостинг подключён: ${connectedCount} ${connectedCount === 1 ? "площадка" : "площадки"}` : "Материалы можно подготовить без подключения"}</p><p className="mt-1 text-[10px] leading-4 text-white/28">Для автоматической публикации подключаются VK, Telegram, VC.ru и Одноклассники. Дзен пока получает готовый пакет статьи.</p></div><form action={completeSelfServiceChannelOnboarding} className="mt-3 sm:mt-0"><button className="w-full rounded-xl bg-violet-500 px-5 py-3 text-xs font-semibold text-white transition hover:bg-violet-400 sm:w-auto">{connectedCount ? "Продолжить — собрать месяц" : "Продолжить без подключения"}</button></form></section> : null}
      </div>
    </SelfServiceAppShell>
  );
}
