    ...
    <div className="mt-7 grid gap-4 lg:grid-cols-2">
      {channels.map((channel) => (
        <article key={channel.platform} className={`rounded-[22px] border p-5 shadow-[0_24px_80px_rgba(0,0,0,.16)] sm:p-6 ${channel.connected ? "border-violet-400/20 bg-violet-500/[0.055]" : "border-white/[0.07] bg-white/[0.03]"}`}>
          ...
          {channel.connected ? <div className="mt-5 rounded-2xl border border-violet-400/12 bg-black/15 p-4"><div className="flex items-center gap-2"><span className="grid h-5 w-5 place-items-center rounded-full bg-violet-500/15 text-[10px] text-violet-200">✓</span><p className="text-xs font-semibold text-white/70">{channel.title || channel.reference}</p></div><p className="mt-2 text-[10px] text-white/28">{channel.credentialHint ? `Доступ сохранён: ${channel.credentialHint}` : "Соединение проверено и готово к публикации"}</p>{channel.platform === "vk" ? <Link href={`/api/integrations/vk/connect?reference=${encodeURIComponent(channel.reference)}`} className="mt-4 flex w-full items-center justify-center rounded-xl bg-[#2787F5] px-4 py-3 text-xs font-semibold text-white transition hover:brightness-110">Разрешить публикацию с фотографиями через VK →</Link> : null}</div> : <div className="mt-5 rounded-2xl border border-white/[0.05] bg-black/10 px-4 py-3"><p className="text-[10px] leading-4 text-white/28">Можно пропустить сейчас. Тексты и визуалы всё равно будут созданы в кабинете.</p></div>}
            ...
            {channel.platform === "vcru" ? <>
              ...
              {channel.platform === "vk" ? <div className="rounded-2xl border border-[#2787F5]/25 bg-[#2787F5]/[0.08] p-4 sm:p-5">
                ...
                <div className="mt-3 rounded-xl border border-emerald-400/15 bg-emerald-400/[0.06] p-3">
                  ...
                  <p className="mt-2 text-[9px] leading-4 text-white/25">После подключения Ribes отдельно запросит у администратора разрешение на передачу фотографий.</p>
                </div>
