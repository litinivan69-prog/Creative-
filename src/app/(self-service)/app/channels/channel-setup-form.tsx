    ...
    <div className="mt-7 grid gap-4 lg:grid-cols-2">
      {channels.map((channel) => (
        <article key={channel.platform} className={`rounded-[22px] border p-5 shadow-[0_24px_80px_rgba(0,0,0,.16)] sm:p-6 ${channel.connected ? "border-violet-400/20 bg-violet-500/[0.055]" : "border-white/[0.07] bg-white/[0.03]"}`}>
          ...
          <form action={channel.platform === "vk" && channel.oauthAvailable && !channel.connected ? "/api/integrations/vk/connect" : connectSelfServiceSocialChannel} method={channel.platform === "vk" && channel.oauthAvailable && !channel.connected ? "get" : undefined} className="mt-5 space-y-3">
            ...
            <button className="w-full rounded-xl bg-violet-500 px-5 py-3 text-xs font-semibold text-white transition hover:bg-violet-400">{channel.connected ? "Проверить и обновить" : channel.platform === "vk" && channel.oauthAvailable ? "Войти через VK" : "Подключить"}</button>
          </form>
