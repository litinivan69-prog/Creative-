        ...
        <article key={channel.platform} className={`rounded-[22px] border p-5 shadow-[0_24px_80px_rgba(0,0,0,.16)] sm:p-6 ${channel.connected ? "border-violet-400/20 bg-violet-500/[0.055]" : "border-white/[0.07] bg-white/[0.03]"}`}>
          ...
          <form action={channel.platform === "vk" && channel.oauthAvailable ? "/api/integrations/vk/connect" : channel.platform === "ok" && channel.oauthAvailable ? "/api/integrations/ok/connect" : connectSelfServiceSocialChannel} method={(channel.platform === "vk" && channel.oauthAvailable) || (channel.platform === "ok" && channel.oauthAvailable) ? "get" : undefined} className="mt-5 space-y-3">
            ...
            {channel.platform === "vcru" ? <>
              ...
              {channel.platform === "vk" ? channel.oauthAvailable ? <div className="rounded-2xl border border-[#2787F5]/25 bg-[#2787F5]/[0.08] p-4 sm:p-5"><p className="text-sm font-semibold text-white/85">Подключение VK одним входом</p><p className="mt-1 text-[10px] leading-4 text-white/38">Укажите ссылку на сообщество и нажмите кнопку ниже. VK попросит войти, выбрать аккаунт и разрешить Ribes публиковать записи и фотографии. Ключи создавать и копировать не нужно.</p></div> : <div className="rounded-2xl border border-[#2787F5]/25 bg-[#2787F5]/[0.08] p-4 sm:p-5"><p className="text-sm font-semibold text-white/85">Настройка VK один раз</p><p className="mt-1 text-[10px] leading-4 text-white/38">Сначала: Управление → Сообщения → включите «Сообщения сообщества». Затем: Управление → Дополнительно → Работа с API → Создать ключ. У ключа включите «Управление», «Фотографии», «Стена» и «Сообщения сообщества».</p><Link href="https://vk.com/groups?tab=admin" target="_blank" className="mt-3 inline-flex text-[10px] font-semibold text-[#70b1ff]">Открыть мои сообщества VK →</Link></div> : null}
            ...
            <button disabled={channel.approvalPending} className="w-full rounded-xl bg-violet-500 px-5 py-3 text-xs font-semibold text-white transition hover:bg-violet-400 disabled:cursor-not-allowed disabled:bg-white/[0.06] disabled:text-white/30">{channel.platform === "vk" ? channel.connected ? "Обновить доступ через VK" : "Войти через VK и подключить" : channel.connected ? "Проверить и обновить" : channel.approvalPending ? "Ожидаем одобрение ОК" : channel.platform === "ok" && channel.oauthAvailable ? "Войти через Одноклассники" : "Подключить"}</button>
          </form>
