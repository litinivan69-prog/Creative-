  ...
  return (
    <form action={saveSelfServiceMaterialText} className="space-y-3">
      ...
      <textarea
        ...
        className="min-h-[420px] w-full resize-none overflow-hidden rounded-[20px] border border-white/[0.07] bg-black/20 px-5 py-5 text-[15px] leading-7 text-white/72 outline-none transition placeholder:text-white/20 focus:border-violet-400/35 focus:bg-black/30 focus:ring-4 focus:ring-violet-500/[0.06]"
      />
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button type="button" onClick={copyText} className="rounded-xl border border-white/[0.07] bg-white/[0.035] px-4 py-2.5 text-xs font-semibold text-white/52 transition hover:bg-white/[0.06] hover:text-white/80">{copied ? "Скопировано" : "Скопировать"}</button>
