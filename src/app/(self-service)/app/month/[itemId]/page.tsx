...
export default async function SelfServiceMaterialPage({
  ...
  // Image generation can finish after a previous confirmation attempt. In that
  // case the old query-string error must not keep claiming that a now-complete
  // material is unfinished.
  const materialBecameReady = query.error === "material_not_ready" && Boolean(body) && visualReady;
  const visibleError = materialBecameReady ? null : query.error;
  ...
  return (
    ...
    >
      <div>
        ...
        {query.notice === "republished" ? <div className="mb-5 rounded-2xl border border-violet-400/15 bg-violet-500/10 px-4 py-3 text-xs font-semibold text-violet-100">Материал опубликован повторно. Новая ссылка сохранена.</div> : null}
        ...
        {query.notice === "text_rewritten" ? <div className="mb-5 rounded-2xl border border-violet-400/15 bg-violet-500/10 px-4 py-3 text-xs font-semibold text-violet-100">Текст переписан как полноценная публикация. ТЗ для слайдов осталось только внутри производства визуалов.</div> : null}
        ...
        {materialBecameReady ? <div className="mb-5 rounded-2xl border border-violet-400/15 bg-violet-500/10 px-4 py-3 text-xs font-semibold text-violet-100">Все изображения готовы. Теперь материал можно подтвердить.</div> : null}
        {visibleError ? <div className="mb-5 rounded-2xl border border-rose-400/15 bg-rose-500/10 px-4 py-3 text-xs text-rose-100">{visibleError === "text_not_ready" ? "Текст пока готовится." : visibleError === "text_rewrite_failed" ? "Не удалось переписать текст с первого раза. Старый вариант сохранён — попробуйте ещё раз." : visibleError === "material_not_ready" ? "Сначала дождитесь текста и всех изображений." : visibleError === "credits" ? "Не хватает кредитов для внесения правок в визуал." : visibleError === "revision_instruction" ? "Напишите, что именно нужно исправить — от 5 до 1000 символов." : visibleError === "visual_missing" ? "Не удалось найти исходный визуал." : visibleError === "visual_revision_failed" ? "Не удалось внести правки. Кредиты не списаны — попробуйте ещё раз позже." : visibleError === "confirm_first" ? "Сначала подтвердите готовность материала." : visibleError === "publication_missing" ? "Публикация ещё не добавлена в календарь." : visibleError === "publication_url_invalid" ? "Проверьте ссылку на опубликованный материал." : visibleError === "manual_export_only" ? "Для этой площадки пока используйте скачивание и ручное размещение." : visibleError === "schedule_invalid" ? "Проверьте дату и время публикации." : visibleError === "already_published" ? "Опубликованный материал уже нельзя перепланировать." : visibleError}</div> : null}
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.12fr)_minmax(340px,.88fr)]">
          <section className={`${darkCardClass} p-5 sm:p-6`}>
            ...
            <MaterialEditor itemId={item.id} initialBody={body} ready={Boolean(body)} article={isArticle} carousel={slides.length > 0} />
          </section>
