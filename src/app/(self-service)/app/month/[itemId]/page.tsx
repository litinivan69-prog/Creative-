      ...
      <div>
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.12fr)_minmax(340px,.88fr)]">
          ...
          <aside className="space-y-4">
            ...
            {article ? <section className={`${darkCardClass} p-5`}><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-violet-300">Экспорт статьи</p><p className="mt-2 text-sm font-semibold text-white/75">Документ для редактора</p><p className="mt-1 text-[10px] leading-4 text-white/28">Заголовки, текст, FAQ и иллюстрации собираются в DOCX.</p></div><span className="rounded-lg bg-white/[0.04] px-2 py-1 text-[9px] text-white/30">DOCX</span></div><a href={`/api/self-service/articles/${article.id}/docx`} className="mt-4 block w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-center text-xs font-semibold text-white/65 transition hover:bg-white/[0.07] hover:text-white">Скачать статью</a></section> : null}
