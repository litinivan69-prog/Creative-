        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)]">
          ...
          <aside className="space-y-5">
            <section className="rounded-[28px] border border-white bg-white p-5 shadow-[0_22px_70px_rgba(77,61,112,0.07)]">
              <div className="flex items-center justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[0.14em] text-violet-600">Визуал</p><h2 className="mt-2 text-base font-semibold text-slate-950">{slides.length > 0 ? `Карусель · ${visuals.length}/${slides.length}` : "Изображение материала"}</h2></div>{visuals[0] ? <a href={visuals[0].src} download className="rounded-full border border-slate-200 px-3 py-2 text-[11px] font-semibold text-slate-600">Скачать</a> : null}</div>
              {visuals.length > 0 ? (
                <div className={`mt-5 grid gap-3 ${visuals.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
                  {visuals.map((visual, index) => <div key={visual.id} className="relative overflow-hidden rounded-[20px] border border-slate-100 bg-slate-50"><img src={visual.src} alt={`Визуал ${index + 1}`} className="aspect-square h-full w-full object-cover" />{visuals.length > 1 ? <span className="absolute bottom-2 right-2 rounded-full bg-white/90 px-2 py-1 text-[10px] font-bold text-slate-600">{index + 1}</span> : null}</div>)}
                </div>
              ) : (
