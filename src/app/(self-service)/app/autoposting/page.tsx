...
function publicationState(publication: { publishStatus: string | null; status: string; publishErrorMessage: string | null }) {
  if (publication.publishStatus === "published") return { label: "Опубликовано", className: "text-violet-200" };
  if (publication.publishStatus === "publishing") return { label: "Публикуется", className: "text-violet-200" };
  if (publication.publishStatus === "failed") return { label: "Нужен повтор", className: "text-rose-300" };
  if (publication.status === "ready") return { label: "В очереди", className: "text-violet-200" };
  return { label: "Ждёт подтверждения", className: "text-white/28" };
}
...
export default async function SelfServiceAutopostingPage() {
      ...
      client: {
        include: {
          ...
          scheduledPublications: {
            ...
            take: 5,
            select: { id: true, scheduledDate: true, scheduledTime: true, platformName: true, topic: true, status: true, publishStatus: true, publishErrorMessage: true },
          },
        },
      },
    },
  ...
  const rules = [
    ...
    { title: "Требовать подтверждение", description: "Без нажатия «Готово к публикации» материал не выйдет.", active: true },
  ];
  ...
  return (
    ...
    >
      ...
      <section className={`${darkCardClass} mt-4 overflow-hidden`}><div className="border-b border-white/[0.06] px-5 py-4"><h2 className="text-sm font-semibold">Ближайшие публикации</h2><p className="mt-1 text-[10px] text-white/30">Очередь строится из вашего календаря.</p></div>{workspace.scheduledPublications.length ? <div className="divide-y divide-white/[0.05]">{workspace.scheduledPublications.map((publication) => { const state = publicationState(publication); return <article key={publication.id} className="grid gap-3 px-5 py-4 sm:grid-cols-[130px_90px_minmax(0,1fr)_110px] sm:items-center"><span className="text-xs text-white/55">{publication.scheduledDate}<span className="ml-1 text-white/22">{publication.scheduledTime || "11:00"}</span></span><span className="w-fit rounded-full bg-white/[0.05] px-2.5 py-1 text-[9px] font-semibold text-white/45">{platformLabel(publication.platformName)}</span><div className="min-w-0"><p className="truncate text-xs text-white/72">{publication.topic}</p>{publication.publishStatus === "failed" && publication.publishErrorMessage ? <p className="mt-1 truncate text-[9px] text-rose-300/65">{publication.publishErrorMessage}</p> : null}</div><span className={`text-[9px] ${state.className}`}>{state.label}</span></article>; })}</div> : <div className="px-5 py-10 text-center"><p className="text-sm font-medium text-white/65">Публикаций в очереди пока нет</p><p className="mt-2 text-xs text-white/28">После подготовки месяца они появятся здесь автоматически.</p><Link href="/app/month" className="mt-5 inline-flex text-xs font-semibold text-violet-300">Открыть календарь →</Link></div>}</section>
    </SelfServiceAppShell>
  );
}
